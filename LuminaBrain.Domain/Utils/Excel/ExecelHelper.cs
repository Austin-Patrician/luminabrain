    ...
    public class ExecelHelper
    {
        ...
        public static IEnumerable<TResult> ExcelToList<TResult>(Stream stream) where TResult : new()
        {
            var propertyInfos = typeof(TResult).GetProperties(BindingFlags.Public | BindingFlags.Instance)
                ...
                .OrderBy(p => p.GetCustomAttribute<ExecelPropertyAttribute>().Order).ToArray();
                    ...
                    if (sheet != null)
                    {
                        ...
                        if (rowCount > 0)
                        {
                            ...
                            for (int i = startRow; i <= rowCount; ++i)
                            {
                                ...
                                for (int j = row.FirstCellNum; j < cellCount; ++j)
                                {
                                    var execlPropertyAttribute =
                                        propertyInfos[j].GetCustomAttribute<ExecelPropertyAttribute>();
        ...
        public static IEnumerable<TResult> ExcelToListFileName<TResult>(Stream stream, string fileName)
            where TResult : new()
        {
            var propertyInfos = typeof(TResult).GetProperties(BindingFlags.Public | BindingFlags.Instance)
                .Where(p => p.CustomAttributes.Count() > 0)
        ...
        public static byte[] ListToExcel<T>(T[] data, string sheetName = "Sheet0")
        {
            ...
            for (int c = 0; c < columnCount; c++)
            {
                ...
                cell.SetCellValue(propertyInfos[c].GetCustomAttribute<ExecelPropertyAttribute>().DisplayName);
            }
        ...
        public static void SetCellDropdownList(IWorkbook workbook, ISheet sheet, List<string> ddlList, string sheetname,
            int sheetIndex, int firstcol, int lastcol)
        {
            ...
            XSSFDataValidationConstraint dvConstraint =
                (XSSFDataValidationConstraint)dvHelper.CreateExplicitListConstraint(datas);
